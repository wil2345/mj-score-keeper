...
*   **Version Alignment Mandate:** To ensure the update logic triggers correctly, the **Landing Page version string** (e.g., `v1.1.3`) and the **Service Worker `CACHE_NAME`** (e.g., `mahjong-score-v1.1.3`) **MUST** always be updated in tandem. This alignment acts as the "cache buster" that signals a new release to the browser.
...
<body class="bg-slate-50 p-4 md:p-8 text-gray-800 print:bg-white [color-adjust:exact] [-webkit-print-color-adjust:exact]">
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="bg-gray-800 text-white shadow-xl rounded-xl p-8 border border-gray-300">
            <h2 class="text-3xl font-black mb-8 flex items-center gap-3"><span class="bg-blue-600 p-2 rounded-lg">🤖</span> AI Strategic Analysis</h2>
        ...
        for p, comment in ai_comments.items():
            ...
            clr_border = "border-green-500" if net >= 0 else "border-red-500"
            clr_text = "text-green-600" if net >= 0 else "text-red-600"
            
            # Format comment with paragraph breaks
            formatted_comment = comment.replace("\n\n", '</p><p class="text-gray-600 leading-relaxed mb-4">')
            
            html += f'''
                <div class="bg-white p-6 rounded-xl border-t-4 {clr_border} flex flex-col h-full shadow-md">
                    <div class="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                        <h3 class="text-2xl font-black tracking-tight text-gray-800">{p_upper}</h3>
                        <div class="flex flex-col items-end">
                            <span class="text-xs font-bold uppercase tracking-widest text-gray-400">Total Net</span>
                            <span class="text-xl font-black {clr_text}">{net:+.1f}</span>
                        </div>
                    </div>
                    <div class="text-gray-600 leading-relaxed italic text-sm">
                        <p class="mb-4">{formatted_comment}</p>
                    </div>
                </div>'''
        html += """</div></div><div class="py-8 text-center text-xs text-gray-400 uppercase tracking-widest">Taiwan Mahjong • Analytical Insight Report • Generated 2026</div></div>"""
